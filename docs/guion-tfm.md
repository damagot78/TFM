...
## 8. Capítulo 8 — Despliegue en Vercel

### Quality gates antes de desplegar

Antes de tocar Vercel, se repitieron los cuatro gates que ya venían acompañando cada capítulo: `pnpm lint` (0 avisos), `pnpm test:run` (248 tests en verde), `pnpm build` (`tsc -b && vite build` correcto) y `pnpm audit` (0 vulnerabilidades). Con todo en verde en local, la hipótesis de partida era que el despliegue sería un trámite. No lo fue — y el motivo es precisamente el valor de este capítulo.

*Cómo contarlo en el vídeo:* "todo en verde en local" y "funciona en producción" no son la misma afirmación. Este capítulo es la demostración práctica de esa diferencia.

### Tres bugs que solo existían en producción, no en local

El despliegue se hizo desde el dashboard de Vercel (importando el repo de GitHub, sin `vercel.json`: Vite y el directorio `api/` se autodetectan). Nada más probar el endpoint real (`curl` contra `/api/validate-pin`, la verificación end-to-end que había quedado pendiente desde el capítulo 5), aparecieron tres fallos en cadena, cada uno solo visible una vez desplegado de verdad — el ejemplo más claro del proyecto de por qué "pasa los tests" y "funciona en producción" no son lo mismo.

**1. `ERR_MODULE_NOT_FOUND` — imports relativos sin extensión.** El primer 500 vino de `api/validate-pin.ts` importando `../src/shared/constants/agents` sin extensión: un estilo perfectamente válido para TypeScript en modo `bundler` (el que usa Vite), pero el loader ESM nativo de Node — el runtime real de una función Vercel, que no bundlea el código en un único archivo — no añade extensiones por su cuenta como sí hace un bundler o `require` en CommonJS: resuelve la ruta del import tal cual está escrita, y falla si no existe un archivo con ese nombre exacto. El stack trace del error (lanzado desde `node:internal/modules/esm/resolve` durante la petición real, no durante el build) lo confirma. Corregido el primero, apareció el mismo error para los imports *dentro* del propio directorio `api/` (`./_lib/isPinValid`) — incluso con el archivo `.js` correcto presente al lado, porque el loader nunca llegó a buscar ese nombre: el mismo bug, en dos sitios distintos, encontrado en dos despliegues consecutivos porque el primer intento de arreglo no fue exhaustivo.

**2. La función se quedaba colgada sin responder.** Con los imports ya resueltos, las peticiones dejaron de fallar con 500 y empezaron a agotar el tiempo de espera sin respuesta alguna. El log de Vercel lo explicaba: `export default` devolviendo una `Response` es un contrato que Vercel ha dejado de soportar para el valor de retorno — lo ignora. La firma que exige ahora es un *export nombrado por verbo HTTP* (`export function POST(request)`), el "estilo fetch". El cambio fue mínimo (`export default function handler` → `export async function POST`) porque la firma interna (`Request` → `Response`) ya era la estándar documentada en el capítulo 5 — solo cambió cómo se expone la función, no su lógica.

**3. El build de Vercel reportaba errores de TypeScript que `tsc -b` en local nunca vio.** Con el endpoint ya respondiendo, el log de build seguía marcando errores (`Buffer`, `process`, `node:crypto` no encontrados; imports que "necesitan extensión explícita"). La causa, documentada por el propio Vercel: no soporta *TypeScript Project References*, y solo lee el `tsconfig.json` de la raíz — que en este proyecto era un archivo "solución" vacío (`files: [], references: [...]`), usado solo para que `tsc -b` orqueste `tsconfig.app.json`/`tsconfig.node.json`/`tsconfig.api.json` en local. Vercel, al ignorar `references`, tipaba con TypeScript por defecto (sin `@types/node`, sin la config real). Arreglo de raíz, no de sintomas: se le dieron `compilerOptions` reales al `tsconfig.json` raíz (reflejo de `tsconfig.api.json`), y `tsconfig.api.json` pasó de `moduleResolution: "bundler"` a `module: "nodenext"` — el mismo patrón que ya usaba `tsconfig.node.json` para `vite.config.ts`, porque `api/` corre con el loader ESM nativo de Node en producción, no con un bundler, y por tanto sí exige extensiones reales. Este cambio cierra la brecha del bug 1: a partir de ahora, un import sin extensión en `api/` falla en `tsc -b` en local, no solo en Vercel — se verificó reintroduciendo temporalmente un import sin extensión y confirmando que `tsc -b` lo detecta con el mismo error (`TS2835`) que había reportado Vercel.

*Cómo contarlo en el vídeo:* los tres bugs comparten una misma lección, contada tres veces desde ángulos distintos: **el entorno de desarrollo (Vite, un bundler) y el entorno de producción (el loader ESM nativo de Node en una función serverless) no son el mismo runtime**, aunque compartan el mismo lenguaje y el mismo código fuente. TypeScript en modo `bundler` deja escribir código que compila pero no arranca. La solución no fue "parchear cada síntoma", sino alinear la configuración de `api/` (tanto la de TypeScript como la de exports) con las reglas reales del entorno donde se ejecuta — y, en el caso del tercer bug, además cerrar la brecha para que el propio `tsc -b` local detecte en el futuro esta clase de error antes de llegar a desplegar.

### Usuario y contraseña de prueba: una decisión sobre un repositorio público

El requisito del TFM de documentar credenciales de prueba en el README choca con que el repositorio de GitHub es público: publicar el PIN real de un agente permitiría a cualquiera "identificarse" en la app desplegada. La decisión fue **rotar el PIN de `Agente 1`** a un valor explícitamente pensado para ser público (`1234`, configurado así en las variables de entorno de Vercel, nunca en el código) antes de documentarlo, dejando el resto de agentes (`Agente 2`–`Agente 4`) con PIN reales no publicados. El riesgo residual de exponer ese PIN de demostración es bajo — como ya recoge la nota de seguridad del propio README, el login es un registro interno de qué agente generó cada cotización, no una puerta a datos sensibles — pero rotarlo evita depender de esa evaluación de riesgo para justificar publicar un valor real.

*Cómo contarlo en el vídeo:* documentar credenciales de prueba para un evaluador y mantener buenas prácticas de seguridad no tienen por qué chocar — basta con que el valor documentado sea, desde el origen, un valor pensado para ser público.

### Un bug funcional encontrado usando la app real ya desplegada, no en local

Con el despliegue ya verificado, se probó la aplicación de principio a fin como la usaría el personal del club — y ahí apareció un bug que ningún test ni ninguna sesión de desarrollo en local había detectado: rellenar los datos de un cliente en "Calculadora", cambiar a "Tarifas" o "Exportar" y volver dejaba el formulario completamente vacío.

La causa: `App.tsx` renderiza cada pantalla con `{view === 'quote' && <QuoteForm ... />}` — un `&&` condicional, no una ocultación con CSS. Cambiar de pestaña desmonta `QuoteForm` por completo, y todo el estado del formulario (`useQuoteForm`) es `useState` local: se destruye al desmontar y arranca vacío al volver a montar. Nunca se había notado porque ningún test end-to-end recorre esa navegación concreta, y en desarrollo es fácil no cambiar de pestaña a media cotización.

La solución, coherente con el patrón ya usado dos veces en el proyecto (`tariffOverridesRepository` en el capítulo 6, `exportedQuotesRepository` en el capítulo 7): un `quoteDraftRepository` que persiste el borrador en `sessionStorage` en cada cambio, con la misma sanitización defensiva por campo ante datos corruptos o de forma inesperada. Limpiarlo solo al cambiar de pestaña no bastaba — hacía falta decidir explícitamente **cuándo** borrarlo también, no solo cuándo guardarlo: al cerrar sesión (para que el siguiente agente no vea datos de un cliente ajeno en la misma pestaña del navegador) y tras añadir la cotización a exportar (para que no reaparezca un borrador ya "usado" al empezar la siguiente cotización — sin esto, el formulario se habría quedado mostrando indefinidamente al cliente ya exportado, con el riesgo real de generar la siguiente cotización a su nombre por error).

Implementar el segundo punto reveló un acoplamiento que no era evidente hasta tocarlo: el mensaje "Cotización añadida a la exportación" vivía anidado dentro de la misma condición que el botón de exportar (`canExport`), así que en cuanto el formulario se vaciaba tras exportar, `canExport` pasaba a `false` y el mensaje desaparecía en el mismo instante en que debía mostrarse. Hubo que separar ambos en `QuoteSummary` para que el mensaje pudiera mostrarse sin que hubiera ya nada exportable.

*Cómo contarlo en el vídeo:* es la misma lección del capítulo 8 contada una cuarta vez, pero esta vez sobre UI en vez de sobre infraestructura de despliegue — un test que pasa en local prueba una ruta de código, no el recorrido real que hace un usuario por la aplicación. La navegación entre pestañas es exactamente el tipo de interacción que solo aparece al usar la app de verdad, y encontrarlo en producción (no antes) es precisamente por qué este capítulo incluye "probar la app ya desplegada como la usaría el personal", no solo "verificar que el despliegue responde".

### Verificación

`pnpm lint`, `pnpm test:run` (266 tests: 248 del cierre del despliegue + 18 de este bugfix) y `pnpm build` en verde. Cobertura: 100% funciones, 98,8% líneas, 97,7% ramas. `pnpm audit`: 0 vulnerabilidades. Desplegado en [tfm-two.vercel.app](https://tfm-two.vercel.app); verificado contra el endpoint real (no interceptado, a diferencia de la prueba del capítulo 5) con `curl`: PIN correcto (200, `valid:true`), PIN incorrecto y agente inexistente (401, mismo mensaje genérico), método no permitido (405), payload inválido (400). El bug de pérdida de datos se probó de principio a fin en Chromium real: primero contra `pnpm dev` (con `/api/validate-pin` interceptado, mismo patrón del capítulo 5) y después contra la producción real desplegada — cambio de pestaña conserva los datos, exportar vacía el formulario mostrando la confirmación, y cerrar sesión limpia el borrador.

Pendiente de cerrar el capítulo: slides y vídeo.
